...
(async () => {
  ...
  try {
    ...
    await page.waitForSelector('div.bg-white, div.text-2xl, div.text-3xl, div.text-5xl');
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      ...
      // Obtener pregunta texto (buscando varios selectores con XPath para mayor robustez)
      let qText = '';
      const xpaths = [
        "//div[contains(@class,'bg-white') and contains(@class,'rounded-3xl')]//div[contains(@class,'text-2xl') or contains(@class,'text-3xl') or contains(@class,'text-5xl')]/text()",
        "//div[contains(@class,'bg-white') and contains(@class,'rounded-3xl')]//div[contains(@class,'text-2xl') or contains(@class,'text-3xl') or contains(@class,'text-5xl')]",
        "//div[contains(@class,'text-2xl') or contains(@class,'text-3xl') or contains(@class,'text-5xl')]",
      ];
      for (const xp of xpaths) {
        const els = await page.$$('xpath=' + xp);
        if (els && els.length > 0) {
          try {
            qText = (await els[0].innerText()).trim();
            if (qText) break;
          } catch (e) {}
        }
      }
      ...
      // Listar opciones detectadas
      const optionTexts = [];
      for (const b of optionButtons) {
        try { optionTexts.push((await b.innerText()).trim()); } catch(e) { optionTexts.push(''); }
      }
